...
def exportTempSkin(items=None, verbose=True, tempFilePath=TEMP_FILE_PATH, **kwargs):
    ...
    kwargs : Any additional keyword:args that should be passed to exportSkin,
        based on it's parameters/arguments, asside from what is above.  Note, both
        'filePath' and 'filePaths' args wil be auto-popped since we're overriding
        pathing here.

    2026-04-24 update:
        tempFilePath is normalized/expanded and used consistently for cleanup and
        export.
    """
    if not items:
    ...
    tempFilePath = os.path.abspath(os.path.expanduser(tempFilePath))
    ...
    if os.path.isfile(tempFilePath):
        try:
            ...
            om2.MGlobal.displayError("skinner : Couldn't remove existing temp weights file '%s': %s"%(tempFilePath, err))
            return
    kwargs.pop("filePath", None)
    kwargs.pop("filePaths", None)
...
def importTempSkin(items=None, verbose=True, tempFilePath=TEMP_FILE_PATH, **kwargs):
    ...
    2026-04-24 update:
        tempFilePath is normalized/expanded before filesystem checks/import.
    """
    if not items:
        if not utils.getMeshVertIds():
